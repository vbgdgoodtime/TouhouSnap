...
/* ==================== 区域「随机变形」（卡牌键 `xformR`，现仅 1 费「梅莉」）====================
   ...
   `revealEffectWillChange` 的 `xformR` 分支。
...
/* **「含持续效果」**＝卡面标着「持续」的卡：`og`（持续效果）/ `mul`（持续 · 区域倍率）/
...
function muteNoteHTML(card, locIdx) {
  if (cardSealed(card)) {
    return '<div class="zm-note mute-note seal-note">☯️ 此牌已被「封印」：永久失去卡牌文字 —— 揭示 / 持续 / 时机 / 防护效果一律不发动（战力照常计入；回手、复活、换边、变身都不会恢复）</div>';
  }
  if (locIdx >= 0 && locMuted(locIdx)) {
    return `<div class="zm-note mute-note">🌊 文本已被「${locDef(locIdx).n}」抹除：此牌的揭示 / 持续 / 时机 / 防护效果一律不发动（战力照常计入；离开静海后文本恢复）</div>`;
  }
  return '';
}
...
function revealBlockNoteHTML(card, locIdx) {
  ...
  if (cardSealed(card) || locMuted(locIdx)) return ''; // 整张文本已被抹除（封印 / 静海）⇒ 已有覆盖面更广的提示行
  return `<div class="zm-note noreveal-note">🌑 揭示已被「${locDef(locIdx).n}」封锁：此牌的揭示效果不发动（战力照常计入；持续 / 时机 / 防护效果照常；离开法界后此后的揭示照常发动）</div>`;
}
...
/* ==================== 放大查看（图鉴 / 手牌 / 场上共用一个弹窗）====================
   **卡图为主**：实时费用 / 实时威力就在卡面左上、右上角标上（带升降着色）—— 那是数值的唯一显示处。
   详情区**不复述卡面**（不重复效果文本、不重复费用与威力数字、不列效果分类标签），只补卡面说不了的：
     ① 当前状态（`cardStateNotes`）：费用递减的**实时计数**（只在真的被改动时出现）；
     ② 战力构成（`renderPowerHistory`）：场上已翻开卡才有的那条横向战力度 —— 各行之和＝卡面右上角的实时威力；
     ③ 边界提示（`muteNoteHTML` / `revealBlockNoteHTML`）：封印 / 静海 / 法界封锁。
   卡面没有效果文本时（法术 / 白板）才用 `baseTextOf` 补一句兜底 —— 图鉴里通常只剩这一句。 */
...
/* 详情区拼装收口（三个视图共用）：**当前状态** → **边界提示** → 兜底说明；空的一律不输出。
   数值一律不在这里复述 —— 实时费用 / 实时威力已在卡面左上、右上角标（带升降着色）。
   ⚠️ #powerPanel（战力构成条）是** .zoom-main-col 里的常驻节点**（紧跟在 #zoomInfo 之后），
   由 renderPowerHistory 单独填充 —— 它不是本函数的产物，也不受这里的 innerHTML 重写影响；
   三段全空（多数图鉴卡与手牌卡）时给 #zoomInfo 挂 .no-text 收掉留白（用不了 :empty）。 */
function zoomInfoHTML(opts) {
  const state = (opts.state || []).filter((s) => s.text)
    .map((s) => `<div class="zm-state">${s.text}</div>`).join('');
  const note = opts.note ? `<div class="zm-note">${opts.note}</div>` : '';
  const base = opts.baseText ? `<div class="zm-desc">${opts.baseText}</div>` : '';
  const info = $('zoomInfo');
  if (info) info.classList.toggle('no-text', !state && !note && !base);
  return state + note + base;
}
/* 卡面没有效果文本时（法术 / 白板）给一句兜底；**有文本就返回空串** —— 卡面上已经写着，详情区不再复述一次。 */
...
/* 放大视图的**当前状态**行（手牌 / 场上共用；图鉴按 def 渲染、恒为空数组）：只报费用递减的**实时计数**
   （`costDownP` 场上高战力卡数 / `costDown` 双方摧毁张数）—— 这是全弹窗唯一卡面写不出来的东西。
   ⚠️ 战力侧**不在这里报**：实时威力看卡面右上角标，构成看下面的战力构成条（`renderPowerHistory`，
   只在场上已翻开卡出现），两边都再报一遍就又变成复述。 */
function cardStateNotes(card, locIdx) {
  const def = card.def;
  const out = [];
  if (def.costDownP && state.players[card.side]) {
    const n = fieldPowerCount(card.side, def.costDownP.min);
    const cut = def.costDownP.n * n;
    out.push({ text: `能量消耗 −${cut} · 场上已翻开、威力 ≥ ${def.costDownP.min} 的卡牌 ${n} 张` + endCostNote(cut, def.c) });
  }
  if (def.costDown) {
    const n = destroyCount();
    const cut = def.costDown * n;
    out.push({ text: `能量消耗 −${cut} · 本场战斗双方已被摧毁 ${n} 张牌` + endCostNote(cut, def.c) });
  }
  return out;
}
/* 费用已减到 0 时的边界提示（0 费是下限，再减不再变化）。 */
function endCostNote(cut, c) { return cut >= c ? '（已减到最低 0 费）' : ''; }

// 手牌放大查看：卡面角标已是「实时费用 / 实时威力」（着色），详情区只补当前状态与封印提示
function showHandCard(card) {
  ...
  $('zoomInfo').innerHTML = zoomInfoHTML({
    ...
    note: muteNoteHTML(card, -1),
    baseText: baseTextOf(def),
  });
  renderDeriv(def);
  zoomStageBtn('关闭 ✕');
  $('zoomMask').classList.remove('hidden');
...
/* ==================== 战力影响历史（卡面战力的**构成**）====================
   独立占放大弹窗**顶部一整条**（在卡图之上，不属于详情区），只出现在**场上已翻开卡**查看时
   （手牌 / 图鉴 / 牌池不显示）。形态＝一列**小方块**（高度大于宽度），每枚＝一次战力增减：
   上方是 `±N`、下方是来源（两字以内），横向铺开、来源多时左右滑，不吃弹窗高度。
   末尾另有一枚「合计」＝卡面右上角的实时威力。口径不变：各行之和 = 实时战力 `cardPowerIn`；
   新增来源只需在 powerHistoryRows 追加收集段、并在改动 buff 的结算点调用 addBuffLog。 */
function powerHistoryRows(card, locIdx) {
  const def = card.def;
  const loc = locDef(locIdx);
  const rows = [];
  // 签上只放短标签（来源名），完整来源文本走 tooltip（sub）—— 方块要窄，来源多时靠横向滑动容纳
  if (isSpell(card)) return [{ d: 0, label: '法术', sub: '法术 · 无战力', kind: 'base' }];
  rows.push({ d: def.p, label: '印刷', sub: '卡面印刷威力', kind: 'base' });
...
function renderPowerHistory(card, locIdx) {
  ...
  const box = $('ppRows');
  box.innerHTML = '';
  let run = 0; // 逐枚累计：写进 tooltip（「4 → 8 → 24」），方块上不重复数字
  for (const r of rows) {
    run += r.d;
    const el = document.createElement('span');
    el.className = 'pp-chip ' + (r.kind === 'base' ? 'base' : r.d > 0 ? 'up' : r.d < 0 ? 'down' : '');
    const num = document.createElement('b');
    num.className = 'pp-num';
    num.textContent = (r.kind === 'base' ? '' : r.d > 0 ? '+' : '') + r.d;
    const lab = document.createElement('i');
    lab.className = 'pp-lab';
    lab.textContent = r.label || '效果';
    el.appendChild(num);
    el.appendChild(lab);
    el.title = `${r.sub ? r.sub + ' · ' : ''}${r.kind === 'base' ? '' : (r.d > 0 ? '+' : '')}${r.d} → 累计 ${run}`;
    box.appendChild(el);
  }
  const total = document.createElement('span');
  total.className = 'pp-chip pp-total';
  total.innerHTML = '<b class="pp-num">' + (sum > 0 ? '+' : '') + sum + '</b><i class="pp-lab">合计</i>';
  total.title = '合计战力（＝卡面右上角的实时威力）';
...
function showFieldCard(card, locIdx) {
  ...
  // 失去卡牌文字：封印（永久）∪ 静海（实时，靠地形配色区分）；失去文字 = 卡面效果全部失效，
  // 由下面那条状态提示说明 —— 卡面效果文本照常显示（它写的是这张牌原本有什么）
  const sealed = cardSealed(card);
  const liveCost = cardCost(card);
  const costSign = liveCost > def.c ? 'up' : liveCost < def.c ? 'down' : '';
  const slot = $('zoomCardSlot');
  ...
  $('zoomInfo').innerHTML = zoomInfoHTML({
    ...
    note: muteNoteHTML(card, locIdx) + revealBlockNoteHTML(card, locIdx),
    baseText: baseTextOf(def),
  });
  renderDeriv(def);
