/* =========================================================
   ...
   v178：筛选改为**两层**结构 ——
     ...
     第 2 层「费用」（`#codexFilter` / `#pickFilter`，`.chip`）：
       ...
       （**v185**：原「6 费」档改为 **「6 费+」**＝6 费及以上，用于容纳新增的 **7 费**档「哆来咪」；
        档内顺序仍是「费用↑ → 战力↑ → 卡名字典序」，故 6 费卡在前、7 费卡在后。）
   ...
   注：0-1 档 = 0 费（稗田阿求，暂不入牌库 / 衍生池的幽灵）与 1 费合并。
   ========================================================= */
(function () {
  'use strict';
  ...
  var POOL_TABS = [
    { key: 'normal', label: '普通卡牌池', tip: '人物卡（POOL）：可入卡组，按 0 费及以上分档（现为 0~7 费）' },
  ...
  function defsFor(kind) {
    ...
    return poolDefsOf(kind).sort(orderDefs).filter(function (d) {
      if (f.min != null) return d.c >= f.min; // v185：「6 费+」= 6 费及以上（含 7 费「哆来咪」）
